...
Hoewel we elk melkwegstelsel een object van het type `GType` toekennen (dat de property `MyGType` heeft), herkent de debugger het object `theGalaxy` niet als een object van het type `GType`. Wat gebeurt er? De `G`