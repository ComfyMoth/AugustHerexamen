...
In onze `Main` methode moeten we nu een `IDie` meegeven aan onze `NumberGame` klasse. Ik gebruik in mijn code de `Die` klasse. Alles werkt dan weer zoals het was, met dat verschil dat ik nu via de constructor van ``